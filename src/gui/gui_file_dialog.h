...
GuiFileDialogState InitGuiFileDialog(int width, int height, const char *initPath, bool active, int window)
{
    ...
    state.size.x = width == -1 ? 800 : width;
    state.size.y = height == -1 ? 400 : height;
...
void GuiFileDialog(GuiFileDialogState *state, int window)
{
    if (state->fileDialogActive)
    {
        ...
        if (GuiTextBox((Rectangle){ state->position.x + 10, state->position.y + 35, (float)winWidth - 65, 25 }, state->dirPathText, 256, state->dirPathEditMode))
        {
        ...
        GuiSetStyle(TEXTBOX, TEXT_ALIGNMENT, GUI_TEXT_ALIGN_LEFT);
        GuiSetStyle(TEXTBOX, TEXT_PADDING, GuiGetStyle(DEFAULT, TEXT_PADDING));
...
# endif
        ...
        GuiLabel((Rectangle){ state->position.x + 10, state->position.y + (float)winHeight - 60, 68, 25 }, "File name:");

        GuiSetStyle(TEXTBOX, TEXT_PADDING, 5);
        if (GuiTextBox((Rectangle){ state->position.x + 85, state->position.y + (float)winHeight - 60, (float)winWidth - 210, 25 }, state->fileNameText, 128, state->fileNameEditMode))
        {
        ...
        state->fileTypeActive = GuiComboBox((Rectangle){ state->position.x + 85, state->position.y  + (float)winHeight - 30, (float)winWidth - 210, 25 }, "All files", state->fileTypeActive);
        GuiLabel((Rectangle){ state->position.x + 10, state->position.y + (float)winHeight - 30, 68, 25 }, "File filter:");

        state->SelectFilePressed = GuiButton((Rectangle){ state->position.x + (float)winWidth - 120, state->position.y + (float)winHeight - 60, 110,
